...
distribution; (2--7) distributions of the six most class-correlated features
(\texttt{word\_freq\_your/you/free}, \texttt{char\_freq\_\$/!},
\texttt{capital\_run\_length\_average}), clipped at the 95th percentile; (8) correlation
heatmap of the ten most class-correlated features; (9--11) class-split boxplots of the
three most correlated features; (12) scatter of the two most correlated features,
coloured by class. Exported at 600 DPI as \texttt{figures/00\_eda\_summary.eps} and
reproduced here as PNG.}
\end{figure}

\textbf{Inference (subplot 1, class distribution):} The dataset shows a moderate class
imbalance (60.6\% ham, 39.4\% spam). Stratified splitting was used throughout so both the
linear and kernel models see the same class proportions in every train/test partition and
cross-validation fold.

\textbf{Inference (subplot 8, correlation heatmap):} \texttt{char\_freq\_\$} and
\texttt{char\_freq\_!} correlate most strongly with the spam label among this feature
subset, echoing the pattern already seen in Experiment 2: punctuation associated with
urgency or monetary offers is a strong spam signal. No pair reaches problematic
multicollinearity, so all 57 features were retained for both models.

\textbf{Inference (subplots 2--7, feature distributions):} All six features are strongly
right-skewed with a long tail, which is why standardization (not just mean-centering) was
applied before fitting Logistic Regression and SVM; both are sensitive to feature scale,
unlike the tree-free Na\"ive Bayes models used in Experiment 2.

\textbf{Inference (subplots 9--11, boxplots):} Spam emails have visibly higher medians
and wider interquartile ranges for these features, confirming that a linear decision
boundary on standardized features already has strong discriminative signal to work with,
consistent with the high accuracy achieved by plain Logistic Regression below.
